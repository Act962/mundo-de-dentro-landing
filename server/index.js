...
try {
  ...
  try {
    ...
    await fs.writeFile(CONTENT_PATH, JSON.stringify({ hero: {}, team: {} }, null, 2), 'utf-8');
  }
}
...
app.get('/api/content', async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Erro ao ler conteúdo', error: error.message });
  }
});
...
app.put('/api/content', authenticateToken, async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Erro ao salvar conteúdo', error: error.message });
  }
});
